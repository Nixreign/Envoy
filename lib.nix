...
let
  ...
  # Evaluate a raw `sources` attrset through `sourceType` so shorthand keys
  # expand and fetch inference runs. Used outside NixOS module eval (e.g.
  # by `mkWriteSources`) where we still need the post-eval form.
  evalSources =
    rawSources:
    (lib.evalModules {
      modules = [
        {
          options._envoy = lib.mkOption { type = lib.types.attrsOf sourceType; };
        }
        { _envoy = rawSources; }
      ];
    }).config._envoy;
